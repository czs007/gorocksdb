...
func TestDBGetApproximateSizes(t *testing.T) {
	...
	// no ranges
	sizes, _:= db.GetApproximateSizes(nil)
	ensure.DeepEqual(t, len(sizes), 0)

	// range will nil start and limit
	sizes, _ = db.GetApproximateSizes([]Range{{Start: nil, Limit: nil}})
	ensure.DeepEqual(t, sizes, []uint64{0})

	// valid range
	sizes, _ = db.GetApproximateSizes([]Range{{Start: []byte{0x00}, Limit: []byte{0xFF}}})
	ensure.DeepEqual(t, sizes, []uint64{0})
}
...
func TestDBGetApproximateSizesCF(t *testing.T) {
	...
	// no ranges
	sizes, _ := db.GetApproximateSizesCF(cf, nil)
	ensure.DeepEqual(t, len(sizes), 0)

	// range will nil start and limit
	sizes, _ = db.GetApproximateSizesCF(cf, []Range{{Start: nil, Limit: nil}})
	ensure.DeepEqual(t, sizes, []uint64{0})

	// valid range
	sizes, _ = db.GetApproximateSizesCF(cf, []Range{{Start: []byte{0x00}, Limit: []byte{0xFF}}})
	ensure.DeepEqual(t, sizes, []uint64{0})
}
